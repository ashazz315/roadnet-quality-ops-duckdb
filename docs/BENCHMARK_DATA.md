...
Benchmark 版本由 Golden 版本及三张表内容 hash、全部参数、生成相关源码指纹和算法版本共同确定。故障、路径、GPS 噪声、反馈使用确定性的独立随机流。根清单另记 Git 版本（开发树带 `+dirty`）、源码 hash、依赖版本、操作系统、机器架构、PROJ/GEOS 版本、运行 ID、时区时间与每个产物 SHA-256。同一执行环境重跑比较字节；跨平台或依赖版本不能承诺浮点末位和 Parquet 二进制相同。

已观察到 Windows/Linux 使用相同 Python 包版本时，部分几何/观测文件的字节 hash 仍不同。固定摘要的字节对照只在完整环境标识相同时执行；所有平台都运行两次生成并逐文件比较，也都检查版本、规模、目标覆盖、道路几何距离容差和连续性。不要把跨平台的字节差异直接解释为不同故障或不同业务结果。
